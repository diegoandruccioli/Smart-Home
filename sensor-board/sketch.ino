...
public:
  ...
  int isDark() {
    return analogRead(pin) <= 3000 ? 0 : 1;
  }
  String toJson() {
    // CORRETTO: La parentesi graffa è ora su una nuova riga
...
public:
  Pir(int p, int l) : pirPin(p), ledPin(l) {
    ...
    delay(5000); // Calibrazione PIR
  }
  int getMotion() {
    int motion = digitalRead(pirPin) == HIGH ? 1 : 0;
    // Usa digitalWrite per sicurezza su ESP32
...
void connectToMQTT() {
  ...
  espClient.setInsecure(); // Necessario per HiveMQ su Wokwi
